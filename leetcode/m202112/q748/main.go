...
func main() {
	...
	for _, item := range tests {
		if ans := shortestCompletingWord(item.licensePlate, item.words); reflect.DeepEqual(ans, item.want) {
			fmt.Println(true)
		} else {
...
func shortestCompletingWord(licensePlate string, words []string) string {
	...
	for _, word := range words {
		if isCompletingWord(cnt, count, word) && (ans == "" || len(word) < len(ans)) {
			ans = word
		}
	}
...
func isCompletingWord(cnt [26]int, count int, word string) bool {
	for _, c := range word {
		if cnt['z'-c] > 0 {
			cnt['z'-c]--
			count--
