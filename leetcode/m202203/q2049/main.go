...
func main() {
	...
	for _, item := range tests {
		if ans := countHighestScoreNodes(item.parents); reflect.DeepEqual(ans, item.want) {
			fmt.Println(true)
		} else {
...
func dfs(i int, children [][]int, counts []int) {
	for _, ci := range children[i] {
		dfs(ci, children, counts)
	}
	counts[i] = 1
	for _, ci := range children[i] {
		counts[i] += counts[ci]
