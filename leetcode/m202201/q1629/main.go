...
func main() {
	...
	for _, item := range tests {
		if ans := slowestKey(item.releaseTimes, item.keysPressed); reflect.DeepEqual(ans, item.want) {
			fmt.Println(true)
		} else {
...
func slowestKey(releaseTimes []int, keysPressed string) byte {
	var maxCount = releaseTimes[0]
	var maxKey = keysPressed[0]
	for i := 1; i < len(releaseTimes); i++ {
		t := releaseTimes[i] - releaseTimes[i-1]
		if t > maxCount || t == maxCount && keysPressed[i] > maxKey {
			maxCount = t
